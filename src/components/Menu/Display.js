...
export default function Display({movements}){
    const orderNum = 1;
    const itemsNum = Object.values(movements).reduce((sum, {quantity}) => sum + quantity, 0);
    let subTotal = Object.values(movements).reduce((sum, {total}) => sum + total, 0);
